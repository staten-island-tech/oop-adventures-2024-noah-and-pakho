...
def cutscene1():
        loop.loadingScreen()
        os.system("cls")
        loop.delayPrint(1.5, "Your eyes creep open as you slowly pick yourself up from the ground.")
        loop.delayPrint(1, "Brilliant white pillars of marble rise from the ground, stretching into the sky above.")
        loop.delayPrint(1, "In front of you, a being of massive proportions sits upon a throne of marble. He raises his voice to speak.")
        loop.delayPrint(1, "[???] Hello there.")
        loop.delayPrint(1, "[???] What are you doing here?")
        loop.delayPrint(1, "[You] ...what?") 
        loop.delayPrint(1, "[???] Why are you here? Who are you?") 
        time.sleep(2)
        ...
        loop.delayPrint(1, "[???] Ah.") 
        loop.delayPrint(1, f"[???] '{playerName}'.") 
        loop.delayPrint(1, "[???] Your name is the worst name I have ever heard.")
        loop.delayPrint(1, "[???] Anyways, you seem lost. ") 
        loop.delayPrint(1.4, "[???] Confused, perhaps.") 
        loop.delayPrint(1.5, f"[{playerName}] Yeah... Where am I?")
        loop.delayPrint(1, f"[???] You are currently in my temple. Isn't it so beautiful?") 
        loop.delayPrint(1.5, "'No,' you think to yourself.")
        loop.delayPrint(1, "[???] Well, I must ask you to leave now. I have more important matters to focus on right now.")
        loop.delayPrint(1.5, "No, I won't leave until you give me answers.") 
        loop.delayPrint(1, "Who are you, where am I, and how did I get here?")
        loop.delayPrint(1, "The figure sighs.")
        loop.delayPrint(1.5, "My identity is of no concern to you. Also, I already answered the second question. You're in my tem-")
        loop.delayPrint(1, "' - Who are you???' you interject. He stops speaking.")
        loop.delayPrint(1, "[???] You dare cut off a God while he speaks?")
        loop.delayPrint(1, f"[{playerName}] ... I really... don't care... I just want answers...")
        loop.delayPrint(1.5, "The figure sighs before standing up from his throne. A staff materializes in his hand, and he points it at you.")
        loop.delayPrint(3.5, f"[???] Prepare yourself. Your name will be erased from the annals of history.")
        time.sleep(2)
def cutscene2():
        os.system("cls")
        loop.delayPrint(2.5, "[???] You have been defeated.")
        loop.delayPrint(1.5, "[???] Now leave.")
        loop.delayPrint(2, "Your vision starts to fade to black... ")
        time.sleep(2.5)
        loop.loadingScreen()
        os.system("cls")
        loop.delayPrint(1, "[???] Hello?")
        loop.delayPrint(1, "Your eyes slowly open once more. This time, you lay on a soft bed of grass.")
        loop.delayPrint(1.5, "[???] Oh my, are you okay??")
        loop.delayPrint(1, "[???] You seem to be covered in bruises...")
        ...
        time.sleep(3)
        os.system("cls")
        loop.delayPrint(3, "[???] What happened to you? Your bruises look like they cover your whole body...") 
        loop.delayPrint(1, "[???] Just wait a little longer, one of my friends will be able to help you recover from your wounds.") 
        loop.delayPrint(2.5, "'Thank you,' you tell her. She gives you a smile before you two sit in awkward silence.")
        loop.delayPrint(2, "[???] ...my name is Jade. And what might your name be?") 
        loop.delayPrint(2, f"I'm {playerName}.")
        loop.delayPrint(1.5, "[Jade] I see. A strange name, but you do not seem to be from around here anyways.") 
        loop.delayPrint(1.5, "[Jade] The others should be arriving soon.")
        loop.delayPrint(1.5, "[Jade] The forest surrounding us is filled with dangerous monsters, so they might be caught up with fighting them.")
        loop.delayPrint(4, "The awkward silence returns.") 
        loop.delayPrint(1.5, "[Jade] Now that I think about it, it has been a while since they have went out. I might have to look for them...")
        loop.delayPrint(2, "[Jade] Do not worry, though. You will be in good hands soon.")
        loop.delayPrint(1, "[Jade] Follow me. I will go look for them.")
        loop.delayPrint(2.5, "Jade leads the way as she tries searching for her party.")
        loop.delayPrint(1, "Just then, you hear the rustling of leaves in the distance.")
        loop.delayPrint(2.5, "[Jade] That sounds like commotion. I wonder what is happening?")
